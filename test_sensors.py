...
if __name__ == '__main__':
    ...
    try:
        while(True):
            ...
            # Making a copy of current_time and manipulating it so that the
            # filename can be read in an HTML file without the browser
            # complaining
            current_time = time.strftime('%Y-%m-%d %H:%M:%S')
            time_fh_string = current_time
            time_fh_string.replace(' ', '_')
            time_fh_string.replace(':', '-')
            image_fh = "DW3_DW4-" + time_fh_string + ".jpg"
